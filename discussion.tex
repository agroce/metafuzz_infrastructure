...
its \texttt{CMP} domain.  Such a transform might take, e.g.: \lt{if (x == y)}, where \lt{x} and \lt{y} are integer variables, and rewrite the code 
as:
...
Advancing the effectiveness of fuzzing can improve 
...
Table~\ref{tab:thrusts} shows a broad picture of the responsibility structure for the project.  A key feature of each component is early
...
Collaboration on the fundamental infrastructure and tools will follow the model of the provision of these products to
the community as software hosted on
GitHub.  The project expects to use issues, PRs and, best practice
development (e.g., early investment in CI)
for internal and external efforts.
