...
\clg{TODO: in general, I think this intro is citing us a bit too much/exclusively --- if other
people aren't doing relevant work, why do we need to build research
infrastructure to support them? I'm not
deep enough in the fuzz literature to fix this.  So I'll add notes where I feel
like reference to work by others would help, and someone who knows better can
either kill those todos, or add references and then delete the todos.} 
\clg{This paragraph is one example.}
...
\clg{Anyone besides chen2019 worth citing here?}
...
allocating resources and sharing seeds.\clg{examples?}  This
project\clg{FIXME/note, possibly for others: it's strange that this is the first
time we've introduced the idea of ``this project'', or its goals.  I'm not sure
how to fix this. } aims to make it easy to 
...
widely used fuzzers\clg{which ones? References.} is the introduction of custom mutators:  user-written 
...
bugs \cite{CC22}. \clg{others to cite?} These were implemented in a standalone variant of the 
...
most popular and powerful fuzzers \clg{which ones?}).
...
Finally, the proposed system will included facilities to allow researchers and developers to 
...
\clg{Feels like domain-specificity is missing as a cross-cutting concern in this overview.}
...
\clg{Sorry, this is long, I'm wordy in first drafts.}
...
experience with binary program transformations, and both source-based~\cite{} and binary-only fuzzing~\cite{}.
Their open-source Zipr binary rewriting infrastructure has been used in several
projects to retrofit binaries with security hardening transformations, including
the DARPA Cyber Grand Challenge, and more recently, an NSF project to automatically harden and diversify VM images~\cite{}. 
Zipr provides the technological foundation for ZAFL, their highly-efficient AFL-compatible open source binary fuzzer.
Key to ZAFL's efficacy is the integration and stacking of modular fuzzing-enhancing program transformations.

\clg{everyone else, too!}
...
\paragraph{PI Jack Davdison:}  The most relevant prior NSF support for PI Davdison is  
